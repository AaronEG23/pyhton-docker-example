...
function validateFormLib(pElements, pCallback) {
    ...
    var cm = {
            ...
            $.each(cm.elements, function( pIndex, pValue ) {
                cm.validateElement(pValue, function (pData) {
                    if (pData.status == CONSTANS.OK){
                        ...
                        if (!cm.elements[pIndex].maxChar)         cm.elements[pIndex].maxChar = 1000000000000;
        ...
        mFormVerifyTextInputs           : function (pId, pMinC, pMaxC){
            var val = $("#i"+pId).val();
            if (!pMaxC) pMaxC = val.length;
            if (val && val != cm.nullValue && val.length >= pMinC && val.length <= pMaxC ) {
                $("#i"+pId).attr('class', cm.defaultClass);
            }
            else{
        ...
        verifyNumber: function(pValue) {
            ...
            if (length == val.length){
                $("#i"+pValue.id).attr('class', cm.defaultClass);
                val = cm.mFormVerifyTextInputs(pValue.id, pValue.minChar, pValue.maxChar);
            }
            else{
                $("#i"+pValue.id).attr('class', cm.errorClass);
                val = null;
